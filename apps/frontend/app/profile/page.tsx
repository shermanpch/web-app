...
export default function ProfilePage() {
  ...
  useEffect(() => {
    async function fetchUserData() {
      try {
        ...
        setError("Failed to load some profile data. Please refresh to try again.");
      }
    }
  ...
  const getPlanDisplay = () => {
    ...
    return quota.membership_type.charAt(0).toUpperCase() + quota.membership_type.slice(1);
  };
      ...
      <div className="w-full max-w-5xl mx-auto px-4 py-12">
        ...
        <div className="bg-[#EDE6D6] rounded-2xl p-8 mb-8 shadow-lg w-full">
          ...
          <div className="space-y-2 text-gray-800">
            ...
            <p><span className="font-medium">Plan:</span> {getPlanDisplay()}</p>
            <p><span className="font-medium">Premium Readings left:</span> {quota?.remaining_queries !== undefined ? quota.remaining_queries : '-'}</p>
            <p><span className="font-medium">Member Since:</span> {formatMemberSince(user?.created_at || quota?.created_at)}</p>
          </div>
        </div>
